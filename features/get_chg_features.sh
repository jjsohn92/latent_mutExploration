...
dest="../output/evaluation/features_v3"
...
if [[ "$project" == "all" ]]; then
  ...
  for project in 'Lang' 'Time' 'Closure' 'Cli' 'Collections' 'Codec' 'Compress' 'Csv' 'JacksonCore' 'JacksonXml' 'Jsoup' 'JxPath'
  do
    ...
    if [[ -f $targetFile ]]; then
      cat $targetFile | while read target; do
        python3.9 change_based.py -p $project -b $target -d $dest -o $outputdir
      done
    else
