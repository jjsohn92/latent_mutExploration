...
def main(project:str, bid:int, outputdir:str, prev_feature_df:pd.DataFrame) -> pd.DataFrame:
  ...
  if prev_feature_df is not None:
    ...
    mutinfo_df = mutinfo_df.loc[~mutinfo_df.mutK.isin(processed_mutKs)]
  #
  rows = []
  for fpath, df in mutinfo_df.groupby('fpath'):
  ...
  ret_df = pd.DataFrame(rows, columns = ['mutK', 'mutOp', 'status', 
    'fpath', 'lno', 'mth_or_cons_k', 
    'l_churn', 'l_min_age', 'l_max_age', 'l_n_authors',
    'e_churn', 'e_min_age', 'e_max_age', 'e_n_authors'])
  if prev_feature_df is not None:
    ret_df = pd.concat([ret_df, prev_feature_df], ignore_index=True)
  return ret_df 
...
if __name__ == "__main__":
  ...
  chg_feature_df = main(project, bid, outputdir, prev_feature_df)
